...
{
    public class PersistentDocument : IXmlSerializable
    {
        ...
        public void WriteXml(XmlWriter writer)
        {
            if (Workspaces.Count == 0)
                Atom.ThrowHelper.ThrowSpecificationViolation("An app:service element MUST contain one or more app:workspace elements.");

            writer.WriteStartElement("service", PersistentSpecs.AtomPubV1Namespace);
            writer.WriteAttributeString("xmlns", "atom", null, PersistentSpecs.AtomV1Namespace);
    ...
    public class WorkspaceElement : IXmlSerializable
    {
        ...
        public void WriteXml(XmlWriter writer)
        {
            if (string.IsNullOrEmpty(Title))
                Atom.ThrowHelper.ThrowSpecificationViolation("The app:workspace element MUST contain one 'atom:title' element");

            writer.WriteStartElement("workspace");

            writer.WriteElementString("atom", "title", PersistentSpecs.AtomV1Namespace, Title);
    ...
    public class CollectionElement : IXmlSerializable
    {
        ...
        {
            if (string.IsNullOrEmpty(Title))
                Atom.ThrowHelper.ThrowSpecificationViolation("The app: collection element MUST contain one atom:title element.");
            if (string.IsNullOrEmpty(Uri))
                Atom.ThrowHelper.ThrowSpecificationViolation("The app:collection element MUST contain an 'href' attribute, " +
    ...
    public class AcceptElement : IXmlSerializable
    {
        ...
        public void WriteXml(XmlWriter writer)
        {
            if (string.IsNullOrEmpty(Type))
                Atom.ThrowHelper.ThrowSpecificationViolation("atom:accept element MUST contain value");
